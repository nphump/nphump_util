...
function linear_regression(data) // Fit data in the form [[0,1], [1,3], [4,5]] to a straight line 'y = mx + c'. r2 varies from 0 to 1 (perfect line)
{
	...
	let c = (sum_y - lr.slope * sum_x) / n;
	let r2 = Math.pow(n * sum_xy - sum_x * sum_y, 2) / (n * sum_xx - sum_x * sum_x) / (n * sum_yy - sum_y * sum_y);

	return {m, c, r2};
}

// Adds a timeout and handling of non-network errors to fetch()
function fetch_ex(url, timeout_ms)
{
    return new Promise( (resolve, reject) => {
        let timer = setTimeout(() => reject(url + ": Request timed out"),  timeout_ms);

        fetch(url)
		.then(response => {response.ok ? resolve(response) : reject(url + ": " + response.status)} )
...
module.exports = {Timer, Sleep, Process, Run, is_empty_hash, base64_encode, base64_decode, html_encode,
	random_string, pad_10, nice_date, log, warn, debug, linear_regression, fetch_ex};
/* Module usage:
