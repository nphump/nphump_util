...
function linear_regression(data) // Fit data in the form [[0,1], [1,3], [4,5]] to a straight line 'y = mx + c'. r2 varies from 0 to 1 (perfect line)
{
	let n = data.length;
	let sum_x = 0;
	let sum_y = 0;
...
async function multiplex(fn, parameters, concurrent = 5)
{
    ...
    for (parameter of parameters)
	{
        const promise = fn(parameter)
			.then((res) => {let i = promise_queue.indexOf(promise); 
				results[parameter_queue[i]] = res;
				promise_queue.splice(i, 1); parameter_queue.splice(i, 1);})
			.catch(e => {let i = promise_queue.indexOf(promise); let res = `fn threw exception: ${e}`;
				results[parameter_queue[i]] = res;
				promise_queue.splice(i, 1); parameter_queue.splice(i, 1);})
        promise_queue.push(promise);
		parameter_queue.push(parameter);
        
        if (promise_queue.length >= concurrent) await Promise.race(promise_queue);
    }
...
module.exports = {Timer, Sleep, Process, Run, is_empty_hash, base64_encode, base64_decode, html_encode,
	random_string, pad_10, nice_date, log, warn, debug, linear_regression, multiplex};
/* Module usage:
