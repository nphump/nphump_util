...
(new Timer(5000)).then(() => console.log("5 seconds has elapsed")).then(() => (new Timer(3000))).then(() => console.log(".. now another 3 seconds has elapsed"));
...
proc.then((data) => console.log("Successful: " + data)).catch((data) => console.log("Failed: " + data));
...
async function ls() {
	let result = await new Process("ls");
...
module.exports = {Timer, Process, is_empty_hash, base64_encode, base64_decode, html_encode, random_string, pad_10, nice_date, log, warn, debug};
/* Module usage:
